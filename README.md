...
学号密码只在系统 WebView 中由本人输入给学校统一认证页面，App 不读取、不存储密码；课表/成绩数据仅保存在本机，不上传云端；成绩默认隐藏展示需点小眼睛查看。登录会话由系统 WebView 保存，学号等学籍信息会随成绩抓取缓存在本机用于离线展示；如需终止会话，可在「我的」→「退出教务登录」清除。查看「无课教室」时 App 会直接访问贝壳教学平台（`ustb.smartclass.cn`）的公开接口，该请求只携带一个由平台公开配置算出的签名参数与设备网络出口 IP，不包含学号、姓名或任何本机数据。

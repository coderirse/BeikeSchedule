...
```json
{
  ...
  "ZC": "01111111100000...",           // 34 位周次位图（索引 i = 第 i 周，索引 0 为占位符），1=该周有课
...
设计说明：教务的 `ZC` 34 位周次位图比「起始周+结束周+单双周」模型表达力更强（调课周、间断周都准确），故数据模型直接采用位图；UI 层需要"单/双周"角标时由位图推算即可。无固定时间课程（`XB=99999`、无 `KEY`）`dayOfWeek=0`，在课表页底部单独列表展示。
